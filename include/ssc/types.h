...
/* GENERAL */
/*----------------------------------------------------------------------------*/
typedef bl_uword_d4 ssc_group_id;
enum { ssc_null_id = bl_utype_max (ssc_group_id) };
/*----------------------------------------------------------------------------*/
/* SIMULATOR */
/*----------------------------------------------------------------------------*/
enum ssc_out_type_e {
...
{
  *err = bl_mkerr (bl_memr16_size (d->data));
  *str = bl_memr16_beg_as (d->data, char const);
}
/*----------------------------------------------------------------------------*/
static inline char const* ssc_output_read_as_string(
  ssc_output_data const* d, bl_u16* strlength
  )
{
  bl_assert (bl_memr16_size (d->data));
  *strlength = bl_memr16_size (d->data) - 1;
  return bl_memr16_beg_as (d->data, char const);
}
/*----------------------------------------------------------------------------*/
static inline bl_memr16 ssc_output_read_as_bytes (ssc_output_data const* d)
{
  return d->data;
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static inline bool fiber_is_produce_only (bl_u8 run_flags)
{
  return bl_u8_get_bit (run_flags, ssc_fiber_produce_only);
}
/*----------------------------------------------------------------------------*/
/*this call is not reversible, a produce only fiber can't be reverted to
 produce/consume */
static inline bl_u8 fiber_set_produce_only (bl_u8 run_flags)
{
  return run_flags | bl_u8_bit (ssc_fiber_produce_only);
}
/*----------------------------------------------------------------------------*/
typedef struct ssc_fiber_run_cfg {
  bl_uword max_func_count; /*max recursion count in a time-slice*/
  bl_uword look_ahead_offset_us; /* maximum time that a time-slice can advance
...
static inline ssc_fiber_run_cfg ssc_fiber_run_cfg_rv(
  bl_uword max_func_count, bl_uword look_ahead_offset_us, bl_uword run_flags
  )
{
  ssc_fiber_run_cfg c;
...
typedef struct ssc_fiber_cfg {
  ...
  void*                   fiber_context;
  bl_uword                   min_stack_size; /*bytes*/
  bl_uword                   min_queue_size; /*messages*/
